							...
							$feed = fetch_feed($feedurl);
								...
								for ($i=1;$i<=$maxfeed;$i++)
...

									{
												...
												$insert_query="insert into $table2(feedtitle,feedLink,feedDesc,feedCat,feedImgThumb,isSticky,isHighlight,cat,mcatId)";

											$insert_query.="values('$title','$link','$content',$catId,'$image','0','0','$mcat',$maincatId)";				
...

							{
								...
								foreach($maindata as $value)
...

									{
											...
											$insert_query.="values('$title','$link','$content',$catId,'$picture','0','0','$mcat',$maincatId,'$created_time')";				
											$insert_sql=$wpdb->query($insert_query);
									}
											}
...

							{
								...
								foreach($maindata as $value)

									{
										...
										foreach($incarray as $minc)
										{
											if((strlen($content)>0) && strlen($minc)>0)
											{
										$substr=strchr(strtolower($content),strtolower($minc));
										if(strlen($substr))
											{}

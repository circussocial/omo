...
define('CONSUMER_KEY', '80256651-WoYyVg1qMmvoo5QS6z8mIwfNaafuyrRiVXbsSTHxv');
define('CONSUMER_SECRET', '6FAMC42WYONERl2JiLSO6Z1Of8TWuqTKft3iQfvV84');
/**
...
$data = search_for_a_term($bearer_token, "keepsgclean"); //  search for the work 'test' 
...
$remove[] = "'";
 ...
 $desc = $x['user']['description'];
 $desc = str_replace( $remove, "", $desc );
 $title = $x['text'];
 $title = str_replace( $remove, "", $title );
 $autor = $x['user']['name'];
 $image = $x['user']['profile_image_url'];
 $date  = $x['user']['created_at'];
 $keyword = $x['entities']['hashtags'][0]['text'];
 $source = 'twitter';
 $publish = '1';
	
$con=mysqli_connect("localhost","appscirc_shahid","d%p#@pae","appscirc_neasing");
...
$sql="INSERT INTO appscirc_neasing.tbl_rss_data (id,title, details, author, image, date, source, keyword,publish)
VALUES
('', '$title','$desc','$autor','$image','$date','$source','$keyword','$publish')";

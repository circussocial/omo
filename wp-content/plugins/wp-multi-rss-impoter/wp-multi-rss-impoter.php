...
add_action('savcron_event2', 'savcron_feedDelete');
add_filter('cron_schedules', 'savcron_update_sched');


function savcron_activate() {
	if (!wp_next_scheduled('savcron_event')) {
		wp_schedule_event(current_time('timestamp'), 'fifteenmin', 'savcron_event');
	}
	if (!wp_next_scheduled('savcron_event2')) {
		wp_schedule_event(current_time('timestamp')+300, 'fifteenmin', 'savcron_event2');
	}
...
function savcron_deactivate() {
	...
	wp_clear_scheduled_hook('savcron_event2');
}


function savcron_hourly() {
include('collectdata1.php');
wp_cron_feeds();

}

function savcron_feedDelete()
{
	global $wpdb;
...
function savcron_update_sched( $schedules ) {
	return array(
	    'fifteenmin' => array('interval' => 60*10, 'display' => 'Every Minute'),
...
function collectdata() {
		include_once('collectdata1.php');
		/*?>
		<iframe src="<?=get_option('siteurl');?>/wp-content/plugins/wp-multi-rss-impoter/collectdata1.php" frameborder="0" width="100%" height="100%"></iframe>
		<?php*/
...
document.getElementById("txtHint").innerHTML='<div align="center"><img src="<?= get_site_url(); ?>/wp-content/plugins/wp-multi-rss-impoter/wait.gif" /></div>';
            ...
            xmlhttp.onreadystatechange=function()
            {
                ...
                {
					document.getElementById("txtHint").innerHTML='<img src="<?= get_site_url(); ?>/wp-content/plugins/wp-multi-rss-impoter/wait.gif" />';
            ...
            foreach ($cat_arr as $catrow) {
                ...
                if (!empty($inc) && !empty($excl)) {
                    ...
                    if (!empty($inc)) {
                        ...
                        $incl = "SELECT * FROM $tablename WHERE $feedwere and (isSticky = '1' ) and  mcatId in ($catrow->MCID) order By createdon DESC ";
                        ...
                        $exclude = "SELECT * FROM  $tablename  WHERE $feedwere and (isSticky = '1' ) and mcatId in ($catrow->MCID) order By createdon DESC";
                        $rss_cat_sticky_arr[$as]['image'] = $image;
                        $rss_cat_sticky_arr[$as]['data'] = $wpdb->get_results($exclude);
                    } else {
                        $rss_cat_sticky = 'SELECT * FROM ' . $tablename . ' WHERE (isSticky = "1" ) and  mcatId in (' . $catrow->MCID . ') order By createdon DESC';
                        $rss_cat_sticky_arr[$as]['image'] = $image;
        ...
        foreach ($cat_arr as $catrow) {
            ...
            if (!empty($inc) && !empty($excl)) {
                ...
                if (!empty($inc)) {
                    ...
                    $incl = "SELECT * FROM $tablename WHERE $feedwere  and  mcatId in ($catrow->MCID) order By createdon DESC ";
                    ...
                    $exclude = "SELECT * FROM  $tablename  WHERE $feedwere  and mcatId in ($catrow->MCID) order By createdon DESC ";
                    $rss_sportlight_sticky_arr[$as]['image'] = $image;
                    $rss_sportlight_sticky_arr[$as]['data'] = $wpdb->get_results($exclude);
                } else {
                    $rss_cat_sticky = 'SELECT * FROM ' . $tablename . ' WHERE   mcatId in (' . $catrow->MCID . ') order By createdon DESC';
                    $rss_sportlight_sticky_arr[$as]['image'] = $image;
...
{$divheight =$rss_publish_divheight;}
	$redRight = $siteurl . '/wp-content/plugins/' . basename(dirname(__FILE__)) . '/redRight.png';
	$redLeft = $siteurl . '/wp-content/plugins/' . basename(dirname(__FILE__)) . '/redLeft.png';
    $rsshtml = '';
    $rsshtml .='<style>
  .feedWrapper{ border:2px solid #8a8a8a; margin:0 0 0 0;-webkit-border-radius: 12px;border-radius: 12px; }
.feedHead{background:url('.$redRight.') no-repeat right top; 
...
.feedHead span{ background:url('.$redLeft.') no-repeat -1px 0px; height:64px; display:block; padding:0 0 0 10px}
.feed_rss-output{ padding: 0px 0 20px 0px; position:relative; line-height:120%; margin:0 20px}
.feed_title{margin:0 30px 5px 35px !important;}
...
.feed_body{ color:#666666; font-family:"Trebuchet MS", Arial, Helvetica, sans-serif; font-size:11px; text-align:justify; margin:0 0 0 35px;}
.imagefix{ position:absolute; top:0px; left:0px; width:30px; float:left}
.rssicon {margin-top:20px;}
.isHighlight{ width:12px; float:right; position:absolute; margin:-10px 0 0 0; right:25px;}
.isSticky{width:12px; float:right; position:absolute; margin:-10px 0px 0 0; right:50px;}
.feedFlag{ width:12px; float:right; position:absolute; margin:-10px 0 0 0; right:0px;}
.feedWrap{ height:'.$divheight.'px; overflow:scroll;}
.socialIcons{ border-bottom:1px solid #e3e3e3; height:28px; margin:0 0 10px 0}
.socialIcons ul{ margin:0 !important; padding:0 !important; list-style:none !important}
.socialIcons li{ float:left; margin:0 8px 0 0 !important;}
</style>';
...
$social_facebook = $siteurl . '/wp-content/plugins/' . basename(dirname(__FILE__)) . '/social_facebook.png';
$social_rss = $siteurl . '/wp-content/plugins/' . basename(dirname(__FILE__)) . '/social_rss.png';
$social_twitter = $siteurl . '/wp-content/plugins/' . basename(dirname(__FILE__)) . '/social_twitter.png';
$social_instagram =$siteurl . '/wp-content/plugins/' . basename(dirname(__FILE__)) . '/social_instagram.png';
    $rsshtml .='<div class="feedWrapper"><div class="feedHead">
	<span>' . $titlename . '</span>
</div> 
<div class="socialIcons">
	<ul>
		<li><img src="'.$social_facebook.'" /></li>
		<li><img src="'.$social_rss.'" /></li>
		<li><img src="'.$social_twitter.'" /></li>
		<li><img src="'.$social_instagram.'" /></li>
	</ul>
</div>
<div class="feedWrap">';
    if ($rss_sportlight_sticky_arr) {
        foreach ($rss_sportlight_sticky_arr as $catrow) {
            foreach ($catrow['data'] as $row) {
				...
				if($row->cat != "twitter_search" || $row->cat != "twitter_user" || $row->cat != "instagram")
				{
				$rsshtml .= '<div class="feed_body">' . mtext_decode($row->feedDesc) . '</div>';
				}
				else
				{

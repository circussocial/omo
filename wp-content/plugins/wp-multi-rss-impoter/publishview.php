				...
				if(!empty($inc))
						{
							...
							foreach($inc_arr as $word)
							{
									...
									{
										$feedwere .= "and ( feedDesc  like '%$word%')";	
									}
	...
	foreach ($rowcat['data'] as $row )
			{
				...
				//echo rawurldecode($row->feedImgThumb); 
						if(empty($row->feedImgThumb))
								...
								{
								$feedtitletag = '<a href="'.rawurldecode($row->feedLink).'" target="_blank">'.m_text_decode($row->feedtitle).'</a>';
								}	
						
						echo '<td class="view_plan_name column-view_plan_name"><img src="'.rawurldecode($row->feedImgThumb).'" width="75px" height="75px" ></td>';
						echo '<td class="view_plan_name column-view_plan_name">'.$feedtitletag.'</td>';
						echo '<td class="view_plan_name column-view_plan_name">'.m_text_decode($row->feedDesc).'</td>';
						echo "</tr>";
			}
}
